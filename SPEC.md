...
- แสดง **หน้าต่าง 10 Sprint ที่ยึด Sprint ปัจจุบันเป็นหลัก** — เริ่มที่ `Sprint ปัจจุบัน − 1` (มีบริบทข้างหลัง 1 ช่อง แล้วมองไปข้างหน้า 8 ช่อง) · ถ้าไม่มี Sprint ไหน `Active = Now` จะกลับไปใช้ 10 Sprint ท้ายสุด
- เดิมใช้ `.slice(-10)` ตายตัว พอแผนยาวถึง Sprint 20 หน้าต่างเลื่อนไป Sprint 11-20 แล้ว **Sprint ปัจจุบันหลุดออกนอกสไลด์**
- สีของ header column:
...
`orderPagesByCurrentSprint()` ดันหน้าที่มีงานอยู่ใน Sprint ปัจจุบันขึ้นมาก่อน แล้วตามด้วยหน้าที่เหลือตามลำดับเดิม — เปิดสไลด์มาเห็นของที่กำลังทำก่อนเสมอ

- จับกลุ่มหน้าที่ module ถูกตัดข้ามหน้า (`continued`) ไว้ด้วยกัน ไม่ให้หน้า `(ต่อ)` ไปโผล่ก่อนหน้าแรกของ module นั้น
- ถ้าไม่มี Sprint ปัจจุบันอยู่ในหน้าต่าง จะไม่จัดลำดับใหม่
