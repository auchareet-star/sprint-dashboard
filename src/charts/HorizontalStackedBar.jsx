...
function TotalOnTop({ x, y, width, height, index, statusKey, activeStatuses, data }) {
  ...
  return (
    <text x={x + width + 8} y={y + height / 2} fill="#475569" fontSize={T.chartBarLabel} fontWeight={700} dominantBaseline="central">
      {total}
    </text>
  );
}
      ...
      <BarChart data={dataWithTotal} layout="vertical" margin={{ top: 12, right: 48, left: 24, bottom: 12 }}>
        ...
        {activeStatuses.map((status) => (
          <Bar key={status} dataKey={status} stackId="a" fill={STATUS_COLORS[status]} barSize={64}
            shape={(props) => <RoundedBarShape {...props} statusKey={status} activeStatuses={activeStatuses} dataEntry={dataWithTotal[props.index]} />}>
            <LabelList dataKey={status} position="center" fill="#fff" fontSize={T.chartBarLabel} fontWeight={700} formatter={(v) => (v > 0 ? v : '')} />
            <LabelList dataKey={status} content={(props) => <TotalOnTop {...props} statusKey={status} activeStatuses={activeStatuses} data={dataWithTotal} />} />
          </Bar>
        ))}
      </BarChart>
    </ResponsiveContainer>
